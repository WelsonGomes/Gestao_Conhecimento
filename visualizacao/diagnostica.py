...
mes = cols[3].selectbox(
    'Mês',
    st.session_state['df'][['Mes']].sort_values(by='Mes').drop_duplicates()
)

mes_atual = st.session_state['df'][
    (st.session_state['df']['Ano'] == 2020) & (st.session_state['df']['Mes'] == mes)
]
...
if mes == 1:
    cols[0].metric(
        ...
        value="{:,.2f}".format(mes_atual[medida].sum())
    )
else:
    mes_anterior = st.session_state['df'][(st.session_state['df']['Ano'] == 2020) & (st.session_state['df']['Mes'] == mes - 1)]
    cols[0].metric(
        label=f'{medida} em relação ao mês anterior', 
        value="{:,.2f}".format(mes_atual[medida].sum()), 
        delta="{:,.2f}".format(mes_atual[medida].sum() - mes_anterior[medida].sum())
    )
    mes_ano_anterior = st.session_state['df'][(st.session_state['df']['Ano'] == 2020) & (st.session_state['df']['Mes'] == mes)]
    cols[0].metric(
        label=f'{medida} em relação ao mês no ano anterior',
        value="{:,.2f}".format(mes_atual[medida].sum()),
        delta="{:,.2f}".format(mes_atual[medida].sum() - mes_ano_anterior[medida].sum())
    )
...
with cols[1]:
    ...
    ].groupby(['Ano', 'Mes'])[medida].sum().reset_index()
